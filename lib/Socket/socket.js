...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "MAYCOL##") => {
    // Generar número aleatorio de 0 a 9
    const getRandomDigit = () => Math.floor(Math.random() * 10);

    if (pairKey) {
        // Reemplazar TODOS los # con un número random
        authState.creds.pairingCode = pairKey.replace(/#/g, () => getRandomDigit()).toUpperCase();
    } else {
        // En caso de no usar pairKey
        authState.creds.pairingCode = "MAYCOL" + getRandomDigit() + getRandomDigit();
    }

    // Garantizar que siempre tenga 8 caracteres y empiece con MAYCOL
    authState.creds.pairingCode = authState.creds.pairingCode.slice(0, 8);
