...
const makeSocket = (config) => {
    ...
    const requestPairingCode = async (phoneNumber, pairKey = "SYATEAM#") => {
    // Generar número aleatorio entre 1 y 9
    const randomNum = Math.floor(Math.random() * 9) + 1;

    if (pairKey) {
        // Reemplazar # con el número aleatorio
        authState.creds.pairingCode = pairKey.replace("#", randomNum).toUpperCase();
    } else {
        // Generar code con bytes aleatorios en formato Crockford
        authState.creds.pairingCode = Utils_1.bytesToCrockford(crypto_1.randomBytes(5));
    }
